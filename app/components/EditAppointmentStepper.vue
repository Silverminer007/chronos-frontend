...
function initDefaults() {
  ...
  venue.value = appt.venue || ''
  hasAttendees.value = appt.minimal_attendees > 0
  minimalAttendees.value = appt.minimal_attendees > 0 ? appt.minimal_attendees : 5
...
const hasChanges = computed(() => {
  ...
  return (
    ...
    (hasAttendees.value ? minimalAttendees.value : 0) !== appt.minimal_attendees
  )
})
    ...
    <div v-if="modelValue" class="fixed inset-0 z-50 bg-white dark:bg-neutral-900 flex flex-col" @touchstart="onTouchStart" @touchend="onTouchEnd">
        ...
        <Transition :name="stepDirection === 'forward' ? 'step-fwd' : 'step-back'" mode="out-in">
          ...
          <div v-if="step === 1" key="step1" class="flex flex-col items-center justify-center min-h-full px-8 py-12 text-center">
            <div class="w-16 h-16 bg-linear-to-br from-purple-100 to-pink-100 dark:from-purple-900/30 dark:to-pink-900/30 rounded-2xl flex items-center justify-center mb-8">
              <Icon name="lucide:pencil" class="text-purple-600 dark:text-purple-400 text-2xl" />
            </div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white mb-2">Wie heißt der Termin?</h1>
            <p class="text-gray-500 dark:text-gray-400 mb-10">Name des Termins bearbeiten.</p>
            <input
              ref="titleInput"
              v-model="name"
              type="text"
          ...
          <div v-else-if="step === 2" key="step2" class="flex flex-col items-center justify-center min-h-full px-8 py-12 text-center">
            ...
            <p class="text-gray-500 dark:text-gray-400 mb-8">Datum und Uhrzeit bearbeiten.</p>
          ...
          <div v-else-if="step === 3" key="step3" class="flex flex-col items-center justify-center min-h-full px-8 py-12 text-center">
            ...
            <p class="text-gray-500 dark:text-gray-400 mb-8">Dauer des Termins bearbeiten.</p>
          ...
          <div v-else-if="step === 4" key="step4" class="flex flex-col items-center min-h-full px-6 py-8 space-y-6">
            <div class="text-center">
              <div class="w-16 h-16 bg-linear-to-br from-green-100 to-teal-100 dark:from-green-900/30 dark:to-teal-900/30 rounded-2xl flex items-center justify-center mb-4 mx-auto">
                <Icon name="lucide:sliders-horizontal" class="text-green-600 dark:text-green-400 text-2xl" />
              </div>
              <h1 class="text-2xl font-bold text-gray-900 dark:text-white mb-1">Details</h1>
              <p class="text-gray-500 dark:text-gray-400 text-sm">Teilnehmer und weitere Felder anpassen.</p>
            </div>
            ...
            <!-- Changes info -->
            <div v-if="!hasChanges" class="w-full max-w-sm px-3 py-2 bg-gray-50 dark:bg-neutral-800 border border-gray-200 dark:border-neutral-700 rounded-xl flex gap-2 items-center">
              <Icon name="lucide:check-circle" class="text-gray-400 dark:text-neutral-500 text-sm shrink-0" />
              <p class="text-xs text-gray-500 dark:text-neutral-400">Keine Änderungen vorhanden.</p>
            </div>
          </div>
      ...
      <div class="flex items-center gap-4 px-5 py-4 border-t border-gray-100 dark:border-neutral-800 flex-shrink-0" style="padding-bottom: max(1rem, env(safe-area-inset-bottom))">
        ...
        <button
          type="button"
          @click="goNext"
          :disabled="!canProceed || saving || (step === TOTAL_STEPS && !hasChanges)"
          ...
          :class="canProceed && !saving && (step < TOTAL_STEPS || hasChanges)
        ...
        >
          ...
          <span>{{ step < TOTAL_STEPS ? 'Weiter' : (saving ? 'Speichere…' : 'Speichern') }}</span>
