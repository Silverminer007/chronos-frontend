    ...
    >
      ...
      <div class="p-4 space-y-4">
        ...
        <input
          ...
          v-model="form.name"
          type="text"
          placeholder="Terminname bearbeiten…"
        ...
        <!-- Row 4: Expandable description & venue -->
        <div>
          <button
            type="button"
        ...
        <!-- Changes info -->
        <Transition name="inline-fade">
          <div v-if="hasChanges" class="px-3 py-2 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-xl flex gap-2 items-center">
            <Icon name="lucide:info" class="text-yellow-600 dark:text-yellow-400 text-sm shrink-0" />
            <p class="text-xs text-yellow-800 dark:text-yellow-200">Teilnehmer werden über Änderungen benachrichtigt.</p>
          </div>
        </Transition>
        ...
        <div class="flex items-center justify-end gap-3 pt-1 border-t border-gray-100 dark:border-neutral-700">
          ...
          <button
            ...
            :class="isValid && hasChanges && !saving ? 'bg-linear-to-r from-purple-600 to-pink-500 hover:from-purple-700 hover:to-pink-600 shadow-md active:scale-95' : 'bg-gray-300 dark:bg-neutral-600 cursor-not-allowed'"
          >
            <Icon v-if="saving" name="lucide:loader-circle" class="animate-spin text-sm" />
            <span>{{ saving ? 'Speichere…' : 'Speichern' }}</span>
            <Icon v-if="!saving" name="lucide:check" class="text-sm" />
          </button>
        </div>
      </div>
    </div>
